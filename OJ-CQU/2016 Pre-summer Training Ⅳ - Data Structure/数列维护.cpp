...
void Insert(int pos,int tot)
{
    ...
    Splay(Get_kth(root,pos+1),0); // +1是由于初始化了一个root占1
...
int main()
{
    ...
    #endif
    int t, op, pos, c; scanf("%d", &t);
    while (t --) {
        scanf("%d%d", &n, &q);
        Init();
        while (q --) {
            scanf("%d%d%d", &op, &pos, &n);
            switch(op) {
                case 1: Insert(pos, n); break;
                case 2: Delete(pos, n); break;
                case 3: scanf("%d", &c), Make_Same(pos, n, c); break;
                case 4: Reverse(pos, n); break;
                case 5: printf("%d\n", Get_Sum(pos, n));
            }
        }
    }
